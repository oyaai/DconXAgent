...
agent จะ **ถามก่อนเสมอว่าจะเอาไว้ที่ไหน** โดยขึ้นเป็นปุ่มให้กด 3 ตัวเลือก:
...
จากนั้นจะ **สร้างไฟล์ให้เลยโดยไม่ต้องกดอะไร** (`package.json` → `index.html` → `src/main.jsx` → `src/App.jsx`)
แต่ละไฟล์จะขึ้นการ์ด `+ Created ...` พร้อมปุ่ม **Undo** ถ้าไม่เอาก็กดลบทิ้งได้ทันที
พอเสร็จจะบอกคำสั่งที่ต้องรันต่อ เช่น `npm install` แล้ว `npm run dev`
